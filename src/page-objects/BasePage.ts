...
export class BasePage {
  ...
  async provideSetTimeSchedulingDetails(callDuration) {
    ...
    let currentTime = getCurrentTimeFormated(2)
  ...
  async assertPresenceByText(text, state: 'present' | 'absent') {
    if (state == 'absent') {
  ...
  async assertFileDownloaded(triggerElement) {
    ...
    // await download.saveAs('play/test.csv')
  }
  ...
  async assertSuccessAllert(message) {
    await this.successAlert.waitFor({ state: 'attached' })
