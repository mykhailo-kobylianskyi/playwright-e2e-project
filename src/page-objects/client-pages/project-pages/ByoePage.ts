...
export class ByoePage extends BasePage {
  ...
  async openRateModal() {
    ...
    await this.assertPresenceByText(
      'Additional service fee will be applied',
      'present'
    )
  ...
  async assertRateModal() {
    ...
    await this.assertPresenceByText(
      'platfrom will automatically pay the expert for your call with them. The expert will be paid pro-rata based on the hourly rate that you set for them.',
      'present'
    )
    await this.assertPresenceByText(
      'platfrom will then invoice your organisation for this call. The invoice will be a sum of the expert’s fee and platfrom service fee plus any applicable taxes. The fee to platfrom for calls shorter than 30min is 50 USD; the fee for calls longer than 30min is 100 USD. The service fee is charged in the currency set in your office billing details on the platfrom platform.',
      'present'
    )
  ...
  async assertHowItWorksModal() {
    ...
    await this.assertPresenceByText(
      'We will pay the expert pro-rata and invoice you back that amount plus a small service fee and any applicable taxes. The fee to platfrom for calls shorter than 30min is 50 USD; the fee for calls longer than 30min is 100 USD. The service fee is charged in your preferred currency.',
      'present'
    )
    await this.closeDialogModalIcon.click()
    await expect(this.modalDialog).not.toBeVisible()
  ...
  async assertAggrementClosed(data) {
  ...
  async provideSchedulingDetails(callDuration) {
    ...
    // could be specified via ByName and provided ecurrent time from above
    await this.selectorPickOptionByIndex('Call time (GMT+3)', 1)
